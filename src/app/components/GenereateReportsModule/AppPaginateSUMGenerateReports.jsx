...
const AppPaginateSUMGenerateReports = ({ data = [], fetchData, onPageSet, page, loading }) => {
    ...
    if (reportId) {
      try {
        const response = await axios.delete(
          commonConfig.urls.psAutomationDeleteReportRequest + '/' + reportId,
          {
            headers: {
  ...
  const handleDownload = async (fullPath) => {
    ...
    try {
      const response = await axios(
        `${commonConfig.urls.psAutomationDownloadReport}?file_name=${path}`,
        {
    ...
    <AppBox width="100%" overflow="auto">
      <StyledTable>
        <TableHead>
          <TableRow>
            ...
            <TableCell align="left">NO. of Patients</TableCell>
      ...
      {shouldOpenConfirmationDialog && (
        <AppConfirmationDialog
          ...
          onYesClick={() => handleConfirmationResponse(currentReport.current?.ps_report_id)}
        />
      )}
    </AppBox>
  );
...
export default AppPaginateSUMGenerateReports;
